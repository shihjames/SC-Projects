...
def draw_names(canvas, name_data, lookup_names):
    ...
    for i in range(len(lookup_names)):
        ...
        canvas.create_text(prev_x, prev_y, text=name+' '+rank, anchor=tk.SW, fill=COLORS[i % len(COLORS)])
        for j in range(1, len(YEARS)):
            ...
            canvas.create_line(prev_x, prev_y, x, y, width=LINE_WIDTH, fill=COLORS[i % len(COLORS)])
            canvas.create_text(x, y, text=name+' '+rank, anchor=tk.SW, fill=COLORS[i % len(COLORS)])
