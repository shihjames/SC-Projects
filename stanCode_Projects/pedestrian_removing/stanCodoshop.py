...
def get_average(pixels):
    ...
    for pixel in pixels:
        ...
        green += pixel.green
        blue += pixel.blue
    # List 'rgb' contains three average values.
...
def get_best_pixel(pixels):
    ...
    dist = get_pixel_dist(pixels[0], red_avg, green_avg, blue_avg)
    # Variable 'index' represents the index of pixel in list pixels.
    index = 0
    for i in range(1, len(pixels)):
        # Set condition to renew variables.
        if get_pixel_dist(pixels[i], red_avg, green_avg, blue_avg) < dist:
            # Renew value of dist.
            dist = get_pixel_dist(pixels[i], red_avg, green_avg, blue_avg)
